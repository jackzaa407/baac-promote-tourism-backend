require("dotenv").config();
const axiosInstance = require("./axiosReject");
const jwt = require("../utils/genJwtToExternalAPI")
const MESSAGE = require('../utils/message.json')

const checkref = async (cid, phone, ref) => {
  const result_checkref = await axiosInstance.post(
    process.env.EXTERNAL_URL + MESSAGE.EXTERNAL_API_PATH.CHECKREF_API,
    {
      cid:
        "",
      mobileNumber: "0991234326",
      requestDate: "20200225",
      refCode: "1234567"
    },
    {
      headers: {
        apikey: "tAu08J9Aplzkq2W87C0JqnAJ7J39hx55",
        "content-type": "application/json",
        "Authorization" : "bearer " + jwt()

      }
    }
  );
  return result_checkref.data;
};

module.exports = checkref;
